...
export async function POST(request:Request){
  try{
    ...
    const admin=createAdminClient();
    const {data:id,error}=await admin.rpc('create_community_atomic',{
      p_owner_id:user.id,
      p_name:body.name,
    ...
    if(error||!id)throw error??new Error('community_create_failed');
    const {data:community,error:readError}=await admin.from('communities').select('id,owner_id,name,description,logo_url,banner_url,privacy,tags,created_at').eq('id',id).single();
