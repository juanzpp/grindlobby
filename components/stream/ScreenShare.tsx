...
export default function ScreenShare({isPro=false,gameName="GrindLobby",gameBanner="/lobby-games/grind.svg"}:{isPro?:boolean;gameName?:string;gameBanner?:string}){
  ...
  async function start(){
    ...
    try{
      ...
      await room.localParticipant.publishTrack(video,{source:Track.Source.ScreenShare,simulcast:true,videoCodec:"vp8",degradationPreference:"maintain-framerate",screenShareEncoding:{maxBitrate:selected.bitrate,maxFramerate:Math.min(maxFps,capability.maxFps)}});
