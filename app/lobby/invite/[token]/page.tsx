"use client";

import {useEffect,useState} from "react";
import {useParams,useRouter,useSearchParams} from "next/navigation";
import GrindPortalLoading from "@/components/feedback/GrindPortalLoading";

export default function LobbyInvitePage(){
  const params=useParams<{token:string}>(),router=useRouter(),searchParams=useSearchParams();
  const [error,setError]=useState("");
  const lite=searchParams.get("desktop")==="lite";
  useEffect(()=>{
    let cancelled=false;
    void (async()=>{
      const suffix=lite?"?desktop=lite":"";
      const invitePath=`/lobby/invite/${params.token}${suffix}`;
      const response=await fetch(`/api/lobbies/invite/${encodeURIComponent(params.token)}`,{method:"POST"});
      const body=await response.json().catch(()=>({}));
      if(cancelled)return;
      if(response.status===401){
        document.cookie=`grindlobby_next=${encodeURIComponent(invitePath)}; Path=/; Max-Age=900; SameSite=Lax`;
        router.replace(lite?"/login?desktop=lite":"/login");
        return;
      }
      if(!response.ok||!body.lobbyId){setError(body.error||"Convite indisponível.");return}
      document.cookie="grindlobby_next=; Path=/; Max-Age=0; SameSite=Lax";
      router.replace(`/lobby/${body.lobbyId}${suffix}`);
    })();
    return()=>{cancelled=true};
  },[params.token,router,lite]);
  if(error)return <main className="grid min-h-screen place-items-center bg-black px-6 text-white"><div className="max-w-md text-center"><h1 className="text-xl font-semibold">Convite indisponível</h1><p className="mt-3 text-sm text-white/60">{error}</p><button className="mt-6 rounded-lg border border-white/15 px-4 py-2 text-sm" onClick={()=>router.replace(lite?"/desktop-lite?desktop=lite":"/")}>Voltar ao GrindLobby</button></div></main>;
  return <GrindPortalLoading variant="fullscreen" label="Entrando no lobby"/>;
}
