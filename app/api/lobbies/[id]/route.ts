...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const [{data:lobby},{data:membership}]=await Promise.all([
      ...
      admin.from("lobby_members").select("user_id,role,last_seen_at").eq("lobby_id",id).eq("user_id",user.id).gt("last_seen_at",cutoff).maybeSingle(),
    ]);
    if(!lobby)return noStoreJson({error:"Lobby não encontrado."},{status:404});
    const activeMembership=Boolean(membership);
    const authorized=lobby.visibility==="public"||lobby.owner_id===user.id||activeMembership;
    ...
    return noStoreJson({lobby:{...lobby,game,members:(members??[]).map(member=>({...member,profile:profileMap.get(member.user_id)})),isMember:activeMembership,me:user.id}});
  }catch(error){
