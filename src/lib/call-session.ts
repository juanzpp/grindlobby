...
export const callSession = {
  ...
  attach(lobbyId: string, stream: MediaStream) {
    if (state.micStream && state.micStream !== stream) {
    ...
    state = {
      ...
      startedAt:
        state.lobbyId === lobbyId && state.startedAt
          ? state.startedAt
          : Date.now(),
    };
    emit();
  ...
  setMetrics(patch: Partial<CallMetrics>) {
    state = {
      ...state,
      metrics: { ...state.metrics, ...patch, updatedAt: Date.now() },
    };
    emit();
  },
