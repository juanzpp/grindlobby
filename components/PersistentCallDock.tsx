...
function perceptual(percent:number){const n=Math.max(0,Math.min(1,percent/100));return n*n}
...
function PersistentScreenPreview({session,onOpenLobby}:{session:ActiveVoiceSession;onOpenLobby:()=>void}){
 const videoRef=useRef<HTMLVideoElement>(null),audioRef=useRef<HTMLAudioElement>(null),[volume,setVolume]=useState(initialStreamVolume),[hidden,setHidden]=useState(false);
 const share=useMemo(()=>{
 ...
 useEffect(()=>{const element=audioRef.current,track=share?.audio;if(!element||!track)return;track.attach(element);element.volume=1;const gain=perceptual(volume);track.setVolume(gain);element.muted=gain===0;if(gain>0)element.play().catch(()=>{});return()=>{track.detach(element);track.setVolume(1)}},[share?.audio]);
 useEffect(()=>{const track=share?.audio,element=audioRef.current;if(!track)return;const gain=perceptual(volume);track.setVolume(gain);if(element)element.muted=gain===0},[share?.audio,volume]);
 ...
 return <section className="persistent-screen-mini">
  ...
  <div className="persistent-screen-video"><video ref={videoRef} autoPlay playsInline muted/><audio ref={audioRef} autoPlay playsInline/></div>
