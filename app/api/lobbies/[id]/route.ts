...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}) {
 ...
 const [{data:game},{data:members}]=await Promise.all([
  ...
  admin.from('lobby_members').select('user_id,role,joined_at').eq('lobby_id',id).order('joined_at')
 ])
 ...
 return NextResponse.json({lobby:{...lobby,game,members:(members??[]).map(m=>({...m,profile:pmap.get(m.user_id)})),isMember:ids.includes(user.id),me:user.id}})
}
