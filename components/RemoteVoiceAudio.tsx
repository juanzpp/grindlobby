...
export default function RemoteVoiceAudio({stream,volume,muted}:Props){
 const peerId=getRemoteVoicePeerId(stream)??"unknown";
 ...
 async function useDirectOutput(element:HTMLAudioElement){
  ...
  devLog("remote-audio-context-state",{peerId,state:context.current?.state??"direct-fallback",fallback:true});
 }
 async function resumeAndPlay(){
  ...
  try{await applyOutputDevice();}catch(error){devLog("remote-audio-output-device-failed",{peerId,error:String(error)});}
  if(audioContext?.state==="suspended"){
   try{await audioContext.resume();devLog("remote-audio-context-resumed",{peerId,state:audioContext.state});}
   catch(error){devLog("remote-audio-context-resume-failed",{peerId,error:String(error)});await useDirectOutput(element);}
  }
  if(audioContext?.state!=="running")await useDirectOutput(element);
  else element.volume=0;
  try{await element.play();devLog("remote-audio-play",{peerId,audioMuted:element.muted,audioPaused:element.paused,audioContextState:audioContext?.state??"unavailable"});}
  catch(error){devLog("remote-audio-play-failed",{peerId,error:String(error),audioMuted:element.muted,audioPaused:element.paused,audioContextState:audioContext?.state??"unavailable"});}
 }
 useEffect(()=>{
  const element=audio.current;
  if(!element)return;
    devLog("remote-audio-element-created",{peerId,hasElement:true,streamPresent:Boolean(stream)});
  ...
  const track=tracks[0];
    devLog("remote-track-received",{peerId,readyState:track.readyState,muted:track.muted,enabled:track.enabled,audioTrackCount:tracks.length});
  ...
  devLog("remote-audio-context-created",{peerId,state:audioContext.state});
  ...
  applyOutputDevice().catch(error=>devLog("remote-audio-output-device-failed",{peerId,error:String(error)}));
  devLog("remote-audio-context-state",{state:audioContext.state});
  if(userUnlockedRemoteAudio)resumeAndPlay().catch(error=>devLog("remote-audio-play-failed",{peerId,error:String(error)}));else useDirectOutput(element).then(()=>resumeAndPlay()).catch(error=>devLog("remote-audio-play-failed",{peerId,error:String(error)}));
