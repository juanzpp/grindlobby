...
export function referenceFinalSafetyTransformPlugin(){
  return {
    ...
    transform(code,id){
      ...
      // Keep the final source limited to icons already imported by the desktop
      // entrypoint. These substitutions are visual equivalents in the reference
      // surfaces and avoid runtime/compile failures from stale transform symbols.
      ...
      // Normalize App render signatures after all legacy wiring passes.
      ...
      // An older EventsView can survive because buildProfilePatch occurs before
      // it in some composed variants. Replace it against the stable store marker.
