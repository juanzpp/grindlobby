...
import RemoteVoiceAudio from './RemoteVoiceAudio';
...
export default function LobbyRoom({id,user}:{id:string;user:any}){
 ...
 useEffect(()=>{
  if(lobby?.isMember&&!roomConnected.current){
   ...
   roomExitAnnounced.current=false;
 ...
 useEffect(()=>{
  ...
  const expire=()=>{
   if(expired)return;
   expired=true;
  if(roomConnected.current&&!roomExitAnnounced.current){
   roomExitAnnounced.current=true;
   playAudioEvent('disconnected',loadAudioPreferences());
  }
 ...
 async function leave(){setBusy(true);notifyVoiceLeave();const r=await fetch(`/api/lobbies/${id}/leave`,{method:'POST'});setBusy(false);if(r.ok){if(!roomExitAnnounced.current){roomExitAnnounced.current=true;playAudioEvent('disconnected',loadAudioPreferences())}router.push('/')}else setError('Não foi possível sair.')}
 function copy(){navigator.clipboard?.writeText(location.href);setCopied(true);setTimeout(()=>setCopied(false),1500)}
 if(loading)return <main className="room-shell"><GrindLoading variant="fullscreen" label="Entrando no lobby…"/></main>
 ...
 return <main className="room-shell"><div className="ambient a1"/><div className="room-top"><button onClick={()=>router.push('/')}><ArrowLeft size={17}/> Dashboard</button><GrindLobbyLogo variant="full" size="sm" className="room-brand"/><button onClick={copy}>{copied?<Check size={16}/>:<Copy size={16}/>} {copied?'Copiado':'Convidar'}</button></div>
 ...
 <div className="room-grid"><section className="room-panel"><div className="section-head"><div><small>SQUAD</small><h2>Membros da sala</h2></div><span className="room-count">{lobby.members.length}/{lobby.max_members}</span></div><div className="member-grid">{lobby.members.map(m=>{const voice=voiceMembers.find(item=>item.userId===m.user_id);const peer=remotePeers.find(item=>item.userId===m.user_id);const voiceActive=Boolean(voice?.connected);const speaking=Boolean(voice?.speaking||(voice?.audioLevel??0)>.02);return <article className={`member-card ${speaking?"member-speaking":""}`} key={m.user_id}><div className="member-avatar">{(m.profile?.display_name||m.profile?.username||'?')[0].toUpperCase()}<i className={voiceActive?"on":""}/></div><div className="member-identity"><h3>{m.profile?.display_name||m.profile?.username||'Player'}</h3><p>@{m.profile?.username??'player'}{voiceActive&&<b className={`voice-status ${voice?.status.toLowerCase()||"connected"}`}>{voice?.status||"Connected"}{voice?.microphoneMuted?" · Muted":""}</b>}</p></div><span>{m.role==='owner'?'HOST':m.role.toUpperCase()}</span>{peer&&<div className="remote-voice-controls"><RemoteVoiceAudio stream={peer.stream} volume={peer.volume} muted={peer.muted}/><button onClick={()=>togglePeerMuted(m.user_id)}>{peer.muted?"Desmutar":"Mutar"}</button><label>Volume <input aria-label={`Volume de ${m.profile?.display_name||m.profile?.username||'usuário'}`} type="range" min="0" max="200" value={peer.volume} onChange={e=>setPeerVolume(m.user_id,+e.target.value)}/><b>{peer.volume}%</b></label></div>}</article>})}</div></section>
 <aside className="room-side"><AudioHost enabled={lobby.isMember} onStreamChange={setLocalStream}/>{lobby.isMember?<ScreenShare isPro={user.account_tier==='pro'||user.app_role==='admin'}/>:null}<div className="room-panel"><small>SESSION</small><h2>Pronto para jogar</h2><p>A sala permanece salva mesmo se o criador sair.</p><div className="room-actions">{!lobby.isMember?<button onClick={join} disabled={busy} className="primary justify-center">{busy?<Loader2 size={15} className="animate-spin"/>:<Gamepad2 size={15}/>}Entrar na sala</button>:<button onClick={leave} disabled={busy} className="secondary justify-center"><LogOut size={15}/>Sair da sala</button>}</div></div></aside></div></div></main>
}
