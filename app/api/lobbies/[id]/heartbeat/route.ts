...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    await enforceRateLimit(request,{scope:"lobby-heartbeat",limit:30,windowSeconds:60,subject:user.id});
    const admin=createAdminClient();
    const {data:lobby,error:lobbyError}=await admin.from("lobbies").select("status").eq("id",id).maybeSingle();
    if(lobbyError)throw new Error("lobby_lookup_failed");
    if(!lobby)return noStoreJson({error:"Lobby não encontrado."},{status:404});
    if(lobby.status!=="open")return noStoreJson({error:"Lobby encerrado."},{status:410});
    const {data,error}=await admin.from("lobby_members").update({last_seen_at:new Date().toISOString()}).eq("lobby_id",id).eq("user_id",user.id).select("user_id").maybeSingle();
    ...
    return noStoreJson({error:"Não foi possível atualizar a presença."},{status:500});
  }
}
