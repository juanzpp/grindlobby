...
export async function POST(request:Request,{params}:{params:Promise<{token:string}>}){
  try{
    ...
    const {data,error}=await admin.rpc('redeem_community_invite_atomic',{p_invite_id:invite.id,p_user_id:user.id});
    if(error)throw error;
    if(data==='expired'||data==='unavailable')return noStoreJson({error:'Este convite não está mais disponível.'},{status:409});
    if(data!=='joined'&&data!=='already_member')throw new Error('unexpected_invite_result');
    if(data==='joined')await admin.from('community_posts').insert({community_id:invite.community_id,author_id:user.id,type:'activity',title:`${user.display_name||user.username} entrou para a Community`,body:''});
    return noStoreJson({communityId:invite.community_id,alreadyMember:data==='already_member'});
