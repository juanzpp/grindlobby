...
export default function DesktopLobbyRoom({id,user,mode="standard"}:{id:string;user:LobbyUser;mode?:Mode}){
  ...
  useEffect(()=>{const initial=window.setTimeout(()=>void load(),0);const timer=window.setInterval(()=>void load(),10_000);return()=>{window.clearTimeout(initial);window.clearInterval(timer);loadController.current?.abort()}},[load]);
  ...
  useEffect(()=>{if(!session.connected){const clear=window.setTimeout(()=>setRttMs(null),0);return()=>window.clearTimeout(clear)}let disposed=false;const sample=()=>void getLiveKitMediaRttMs().then(value=>{if(!disposed)setRttMs(value)}).catch(()=>{if(!disposed)setRttMs(null)});sample();const timer=window.setInterval(sample,5000);return()=>{disposed=true;window.clearInterval(timer)}},[session.connected]);
