...
type LobbyMember={userId:string;role:string;joinedAt:string;profile:{id:string;username:string;display_name:string;avatar:string|null}|null;player:HoverPlayerData|null};
type LobbyCard={
...
type DashboardData={
  ...
  account:{level:number;xp:number;avatar:string|null;banner:string|null;frame:string|null;effect:string|null;badge:string|null;cardStyle:string|null};
...
export default function Dashboard({user,initialView="dashboard"}:{user:DashboardUser;initialView?:View}){
  ...
  const account=data?.account??{level:1,xp:0,avatar:null,banner:null,frame:null,effect:null,badge:null,cardStyle:null};
