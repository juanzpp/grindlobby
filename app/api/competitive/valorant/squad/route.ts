...
export async function POST(request:Request){
 try{
  ...
  const {data:existing}=await admin.from('valorant_squad_members').select('user_id').in('user_id',memberIds);if((existing??[]).length)return noStoreJson({error:'Um dos jogadores já pertence a outro squad.'},{status:409});
  ...
  const season=await activeValorantSeason();await ensurePlayerRatings(memberIds);if(season)await admin.from('valorant_squad_ratings').upsert({squad_id:squad.id,season_id:season.id,rating:1000,peak_rating:1000,placements_played:0,wins:0,losses:0},{onConflict:'squad_id,season_id',ignoreDuplicates:true});
  return noStoreJson({squad:await userSquad(user.id)},{status:201});
 }catch(error){if(error instanceof InvalidRequestError)return noStoreJson({error:error.message},{status:400});if(error instanceof z.ZodError)return noStoreJson({error:error.issues[0]?.message||'Dados inválidos.'},{status:400});return noStoreJson({error:'Não foi possível criar o squad.'},{status:500});}
}
