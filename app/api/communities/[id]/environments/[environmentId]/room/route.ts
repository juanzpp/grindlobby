...
export async function POST(request:Request,{params}:{params:Promise<{id:string;environmentId:string}>}){
  try{
    ...
    const [{data:community},{data:environment}]=await Promise.all([
    ...
    if(lobbyId){
      const {data:existing}=await admin.from('lobbies').select('id,status').eq('id',lobbyId).maybeSingle();
      if(!existing||existing.status!=='open'){lobbyId=null;await admin.from('community_environments').update({lobby_id:null,updated_at:new Date().toISOString()}).eq('id',environmentId);}
    }
    if(!lobbyId){
      ...
      // If another concurrent request won, prefer the environment's authoritative lobby and close our duplicate.
      const {data:authoritative}=await admin.from('community_environments').select('lobby_id').eq('id',environmentId).single();
  ...
  }catch(error){if(error instanceof RateLimitExceededError||error instanceof RateLimitUnavailableError)return rateLimitResponse(error);return noStoreJson({error:'Não foi possível abrir este Ambiente.'},{status:500});}
}
