...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const [{data:community,error},{data:members},{count:memberCount},{data:environments},{data:posts},{data:events}]=await Promise.all([
      ...
      admin.from('community_members').select('*',{count:'exact',head:true}).eq('community_id',id),
    ...
    return noStoreJson({
      ...
      events:events??[],
      stats:{members:memberCount??0,online,activeRooms:(environments??[]).filter(e=>Boolean(e.lobby_id)).length},
    });
