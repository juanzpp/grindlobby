...
async function inviteByToken(token:string){
  ...
  const {data,error}=await admin.from('community_invites')
    .select('id,community_id,expires_at,max_uses,uses,revoked,created_by')
    .eq('token_hash',hash)
    .maybeSingle();
...
export async function GET(_:Request,{params}:{params:Promise<{token:string}>}){
  try{
    ...
    if(!invite)return noStoreJson({error:'Este convite não está mais disponível.'},{status:404});
    const admin=createAdminClient();
    const {data:community}=await admin.from('communities').select('id,name,description,logo_url,banner_url,privacy').eq('id',invite.community_id).single();
    const {count}=await admin.from('community_members').select('*',{count:'exact',head:true}).eq('community_id',invite.community_id);
    return noStoreJson({community:{...community,memberCount:count??0}});
  }catch{
    return noStoreJson({error:'Convite inválido.'},{status:404});
  }
}
...
export async function POST(request:Request,{params}:{params:Promise<{token:string}>}){
  try{
    ...
    const admin=createAdminClient();
    const {data:existing,error:existingError}=await admin.from('community_members')
      .select('community_id')
    ...
    const {data:reservation,error:reserveError}=await admin.from('community_invites')
    ...
    if(!reservation)return noStoreJson({error:'Este convite foi usado por outra pessoa. Tente novamente.'},{status:409});

    const {error:memberError}=await admin.from('community_members')
      .insert({community_id:invite.community_id,user_id:user.id,role:'member'});
    if(memberError){
      await admin.from('community_invites').update({uses:invite.uses}).eq('id',invite.id).eq('uses',invite.uses+1);
      throw memberError;
    }
    ...
    return noStoreJson({error:'Não foi possível entrar na Community.'},{status:500});
  }
}
